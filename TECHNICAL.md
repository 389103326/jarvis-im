...
> 最后更新：2026-03-09（v1.0.0 正式版）

## 项目概述

Jarvis IM 是一个类 Discord 风格的实时通讯 Web 应用，支持频道聊天、私信、话题线程、Webhook 机器人、WebRTC 语音/视频通话、定时消息、GIF 发送等功能。

- **服务地址：** http://localhost:3000
- **当前版本：** v1.0.0
- **Bundle 大小：** ~469 KB JS（gzip ~142 KB）/ 42 KB CSS
...
| 表名 | 说明 |
|------|------|
| `users` | 用户账号（含 avatarColor、customStatus、bio、avatar_url） |
...
| `scheduled_messages` | 定时消息（status: pending/sent/cancelled） |

### scheduled_messages 表结构
...
| `message_id` | INTEGER | 发送成功后对应消息 ID（nullable） |
...
| POST | `/avatar` | 上传用户头像（base64，≤2MB） |
| DELETE | `/avatar` | 删除用户头像，恢复字母头像 |
...
| GET | `/link-preview` | URL 链接预览（og: 解析+缓存） |
...
| 路径 | 说明 |
|------|------|
| `/uploads/avatars/:filename` | 用户头像图片（Express static） |
| `/assets/*` | 前端 Vite 构建产物 |
...
| `profile_updated` | avatarColor, customStatus, bio, avatarUrl | 更新用户资料 |
| `schedule_message` | channelId/dmChannelId, content, scheduledAt | 创建定时消息 |
| `cancel_scheduled` | scheduledMessageId | 取消定时消息 |
| `list_scheduled` | channelId | 查询频道待发定时消息 |
| `webrtc_call_offer` | targetUserId, offer, callType | 发起 WebRTC 通话 |
| `webrtc_call_answer` | targetUserId, answer | 回应通话 |
| `webrtc_call_ice_candidate` | targetUserId, candidate | 传递 ICE candidate |
| `webrtc_hangup` | targetUserId | 挂断通话 |
| `webrtc_reject` | targetUserId | 拒绝来电 |
...
| `user_profile_changed` | 用户资料变更广播（含 avatarUrl） |
| `scheduled_messages` | 频道定时消息列表 |
| `scheduled_delivered` | 定时消息发送成功通知给发送者 |
| `webrtc_call_incoming` | 收到通话请求 |
| `webrtc_call_answered` | 对方接听 |
| `webrtc_ice_candidate` | 收到 ICE candidate |
| `webrtc_call_ended` | 通话结束 |
| `webrtc_call_rejected` | 来电被拒绝 |
| `webrtc_call_failed` | 通话发起失败（对方离线） |
...
| 组件 | 说明 |
|------|------|
| `ChatArea.jsx` | 聊天区域主容器（移动端空状态含导航栏） |
| `ChatHeader.jsx` | 频道头部（含搜索、媒体画廊、Webhook 管理、导出、定时消息入口） |
| `MessageList.jsx` | 消息列表（虚拟滚动 `@tanstack/react-virtual`、无限上滚加载、滚动位置记忆） |
| `MessageItem.jsx` | 单条消息（Markdown 渲染、代码块复制、@高亮、链接预览、长消息折叠、线程徽章、已读回执） |
| `MessageInput.jsx` | 输入框（草稿持久化、字数限制 2000、慢速模式倒计时、定时发送 🕐、GIF 🎬 按钮） |
| `FilePreviewPanel.jsx` | 多图预览面板（粘贴/拖拽多图缩略图确认） |
| `ScheduledMessagesPanel.jsx` | 定时消息列表面板（查看/取消待发消息） |
| `GifPicker.jsx` | GIF 选择器（Tenor API，trending + 搜索，双列瀑布流） |
...
| `VoiceCallModal.jsx` | WebRTC 通话弹窗（语音/视频，来电铃声、通话计时） |
| `MediaGallery.jsx` | 媒体画廊（图片+GIF 网格 + 灯箱） |
...
| 组件 | 说明 |
|------|------|
| `Sidebar.jsx` | 侧边栏主容器（移动端触控抽屉，监听 activeType 自动切 Tab） |
| `ChannelList.jsx` | 频道列表（含分类折叠、通知偏好右键菜单） |
| `DMList.jsx` | 私信列表 |
| `MemberSidebar.jsx` | 右侧成员栏（搜索、分组、hover 私信、底部统计） |
...
| 组件 | 说明 |
|------|------|
| `Avatar.jsx` | 头像（支持 avatar_url 图片、在线状态绿点） |
| `BookmarkPanel.jsx` | 书签面板 |
| `CommandPalette.jsx` | 命令面板（`Ctrl+K` 唤出） |
| `KeyboardShortcuts.jsx` | 快捷键说明弹窗 |
| `ToastContainer.jsx` | 全局 Toast（点击触发 action 跳转，6 种类型） |
| `UserSettingsModal.jsx` | 用户设置弹窗（账号/通知/外观 Tab，含头像上传） |
...
- [x] 全局 Toast 通知系统（6 种类型，点击跳转，进度条动画）
...
- [ ] **频道权限管理**：只读频道、仅管理员发言等
- [ ] **i18n 多语言**：前端国际化支持
- [ ] **数据库维护**：定时 WAL checkpoint + VACUUM
- [ ] **WebSocket 心跳监控**：超时主动清理
- [ ] **无障碍访问（ARIA）**：补充 ARIA 标签
- [ ] **消息翻译**：集成翻译 API
...
# 一键启动（自动 npm install + 构建前端）
bash start.sh

# 访问 http://localhost:3000
```

> `client/dist` 已提交到仓库，也可跳过构建直接启动后端：
> ```bash
...
|--------|--------|------|
| `PORT` | `3000` | 服务端口 |
| `JWT_SECRET` | `jarvis-im-secret-change-me-in-production` | JWT 密钥，**生产必须修改** |
...
> ⚠️ **WebRTC 通话**需要 HTTPS 环境（浏览器安全策略），生产必须配置 SSL。
...
| 问题 | 原因 | 解决方法 |
|------|------|---------|
| 3000 端口被占用 | 其他进程占用 | `lsof -i :3000` 查找并 kill |
| WebSocket 连接失败 | Nginx 未配置 Upgrade | 检查 Nginx 的 WebSocket headers |
| 头像上传失败 | uploads 目录权限 | `chmod 755 server/uploads/avatars` |
| WebRTC 无法通话 | 非 HTTPS 环境 | 生产必须配 SSL |
| 前端白屏 | 未执行 build | `cd client && npm run build` |
| bcrypt 编译失败 | 缺少 build-essential | `apt install build-essential` |
