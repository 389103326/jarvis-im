...
## [v1.0.0] 2026-03-09 — 正式发布版 🎉

> 基于 v4.4.0 累积迭代，修复所有已知问题后正式发布。

### Bug Fixes（v1.0.0）

- **移动端侧边栏入口缺失**：空状态页（未选频道时）移动端完全看不到侧边栏入口 → 新增顶部导航栏，左上角汉堡 ☰ 按钮，提示文案针对移动端差异化展示
- **用户列表点击无响应**：点击 👥 用户列表中的用户后，DM 在后台打开但侧边栏 Tab 未切换 → `Sidebar.jsx` 监听 `activeType`，DM 激活时自动切换到"私信"Tab
- **消息通知点击无跳转**：收到 DM 或 @提及的 Toast 通知，点击仅关闭不跳转 → Toast 携带 `action` 回调，点击自动导航到对应会话
- **右侧成员栏 UI 优化**：宽度过窄、样式单调 → 全面重设计（宽度 224→256px、成员搜索、分组标签、hover 私信按钮、在线状态发光效果、底部统计栏）
